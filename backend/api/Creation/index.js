...
/**
 * Mounts an endpoint to the app at a given path
 * @param {string} path A Path to the directory
 * @param {Express} app Express app to mount routes to
 * @param {boolean} isDirectory Wether all files in an directory or a single file should be mounted
 */
const mountEndpoints = (path, app, isDirectory) => {
  if (isDirectory) {
    getFilesInDirectory(path).map(endpoint => {
      endpoint = endpoint.substr(0, endpoint.lastIndexOf("."));
      const module = require(`../routes/${endpoint}`);         
      //app.use(`/api/${module.plural ? `${module.plural}` : `${endpoint}s`}`, module);
      app.use(`/${module.plural ? `${module.plural}` : `${endpoint}s`}`, module);
    });
  } else {
...
/*const createEndpoint = (name, data, auth) => {
  if (!findFileInDirectory(`./backend/api/models/${name.singular}.js`) &&
      !findFileInDirectory(`./backend/api/controllers/${name.singular}.js`) &&
      !findFileInDirectory(`./backend/api/routes/${name.singular}.js`)) {
    createFile2(`./backend/api/models/${name.singular}.js`, model.createModel(capitalize(name.singular), data), () => {
      createFile2(`./backend/api/controllers/${name.singular}.js`, controller.createController({singular: capitalize(name.singular), plural: name.plural}, data.data), () => {
        createFile2(`./backend/api/routes/${name.singular}.js`, route.createRoute({singular: capitalize(name.singular), plural: name.plural}, auth), () => {
...
const createEndpoint = async (name, data, auth) => {
  if (!findFileInDirectory(`./backend/api/models/${name.singular}.js`) &&
      !findFileInDirectory(`./backend/api/controllers/${name.singular}.js`) &&
      !findFileInDirectory(`./backend/api/routes/${name.singular}.js`)) {
    await createFile3(`./backend/api/models/${name.singular}.js`, model.createModel(capitalize(name.singular), data));
    await createFile3(`./backend/api/controllers/${name.singular}.js`, controller.createController({singular: capitalize(name.singular), plural: name.plural}, data.data));
    await createFile3(`./backend/api/routes/${name.singular}.js`, route.createRoute({singular: capitalize(name.singular), plural: name.plural}, auth));
    const app = require('../../app');
    await mountEndpoint(name.singular, app.dynamicRouter);
    return true;
  } else {
    console.log(`Couldn't create API endpoint`);
    return false;
...
const deleteEndpoint = async (name) => {
  await deleteFile2(`./backend/api/models/${name}.js`);
  await deleteFile2(`./backend/api/controllers/${name}.js`);
  await deleteFile2(`./backend/api/routes/${name}.js`);
  const app = require('../../app');
  app.setupRouter();
}
